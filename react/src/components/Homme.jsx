...
import { addWishlest, removeWishlest } from './Redux/wishlestSlice';
...
const ImageGallery = () => {
    ...
    const favories = useSelector((state) => state.wishlests.favories || []); // Ensure favories is initialized as an array
    ...
    const handleFavorite = (listing) => {
        ...
        if (isFavorite) {
            dispatch(removeWishlest(listing));
        } else {
            dispatch(addWishlest(listing));
        }
    };
                            ...
                            {listing.images.map((image, index) => (
                                <SwiperSlide key={index} className="static">
                                    <svg xmlns="http://www.w3.org/2000/svg"
                                        onClick={(e) => { e.preventDefault(); handleFavorite({ ...listing, image: `${baseUrl}${listing.images[0]}` }) }} className="absolute h-[30px] w-[30px] ml-[82%] mt-[5px] transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300" viewBox="0 -960 960 960" fill={Array.isArray(favories) && favories.some(item => item.id === listing.id) ? "#75FB4C" : "#FFFFFF"}>
